...
1. 移动端外业保存, /fieldWork/saveFieldWork,POST
   ...
   * 参数：
	 ...
	 * name：采集人
	 * deviceMac:手机mac
	 * coordinates:点集合,{type(点线面类型):xxx，center(中心点):[longitude(经度),latitude(纬度),elevation(高程)]，description(描述)，delete(删除标记)，alias别名，position(定位点):[longitude经度,latitude纬度,elevation高程], attributes:[]}
   * 返回:
	 * OK,保存成功
	 * DATA_NOEXIST:项目不存在
2. 取得外业线面数据, /fieldWork/fieldWork,GET
   ...
   * 参数：
	 * id，项目id
   * 返回：
	 * OK，外业线面数据,[{id(fieldWorkId):xxx,modifyDate:上传时间,name:上传人，fieldWorkPoints:[{id:xxx,centerCoor:{坐标对象},alias:xxx,type:类型，typeC:xxx,description:xxx,positionCoor:{坐标对象}]}, {...}]
	 * DATA_NOEXIST:项目不存在
