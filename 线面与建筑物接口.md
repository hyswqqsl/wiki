...
11. 新建图形线面剖面属性,/shape/shape/attribute/create,GET
   ...
   * 参数：
      ...
      * 返回：
         ...
         * DATA_NOEXIST: 线面不存在
         * DATA_REFUSE：不是自己的线面
12. 删除图形剖面属性, /shape/shape/attribute/remove, POST
   ...
   * 返回：
      ...
      * DATA_NOEXIST: 线面不存在
      * DATA_REFUSE：不是自己的线面      
13. 编辑图形线面剖面属性, /shape/shape/attribute/edit, POST
   ...
   * 返回：
      ...
      * DATA_REFUSE：不是自己的线面
14. 获取建筑物类型,/shape/build/type/download,GET
   * 返回所有线面类型，返回json数组
   * 参数：无
   * 返回：OK，[{"buildModel","大口井","dkj","水源建筑物",true},{...}]    
15. 建筑物模板下载, /shape/build/template/download,GET
   * 根据选择的建筑物类型下载模板excel，每个sheet是一个类型的模板
   * 参数：
       * types：线面枚举英文名的json:["dkj","tj","..."]
   * 返回：
       * OK,自动保存未excel    
16. 单建筑物上传,/shape/single/uploadBuild,POST
   ...
   * 返回：
       ...
       * COOR_FORMAT_ERROR：上传失败:坐标文件或格式错误;
       * COOR_RETURN_PROMPT: 上传过程有问题，返回相应提示信息，前台显示这些问题描述    
17. 多建筑物上传,/shape/multiple/uploadBuild,POST
   ...
   * 返回：
       ...
       * COOR_FORMAT_ERROR：上传失败:坐标文件或格式错误;
       * COOR_RETURN_PROMPT: 上传过程有问题，返回相应提示信息，前台显示这些问题描述
18. 单建筑物下载, /shape/build/single/download,GET
    * 下载点下的建筑物
    * 参数:
        * coordinateId: 点id
    * 返回:
        ...
        * DATA_NOEXIST: 点不存在
        * DATA_REFUSE：不是自己的线面下的点        
19. 多建筑物下载, /shape/build/multiple/download,GET
    * 下载项目下的所有线面
    * 线面每个点有经纬度，高程，还有高程组
    * 每个sheet是一个线面类型
    * 参数:
