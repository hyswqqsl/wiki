...
- [河长云新架构接口](#%E6%B2%B3%E9%95%BF%E4%BA%91%E6%96%B0%E6%9E%B6%E6%9E%84%E6%8E%A5%E5%8F%A3)
  ...
  - [十八 SectionController, 监测断面](#%E5%8D%81%E5%85%AB-sectioncontroller-%E7%9B%91%E6%B5%8B%E6%96%AD%E9%9D%A2)
  ...
  - [工作台接口](#%E5%B7%A5%E4%BD%9C%E5%8F%B0%E6%8E%A5%E5%8F%A3)
...
## 河长云类结构
![image](https://github.com/hyswqqsl/wiki/blob/master/image/%E6%96%B0%E6%9E%B6%E6%9E%84.jpg)
...
5. 设置平台，/platform/systemAdmin/select,POST
   * **角色：系统管理员**
   * 参数：id：平台id
   * 返回:
	 * OK，设置成功
...
11. 地图取得河湖二级图层数据，/waterResource/second/details/{id},GET
   ...
   * 返回：
     ...
     * DATA_NOEXIST,河流不存在
12. 上传河湖坐标,/riverSegment/systemAdmin/upload/coors,POST
   * **web端使用**
   * **角色：系統管理員**
   * 参数：
       * id,河湖id
       * file,上传的excel文件
   * 返回：
       * OK，上传成功，返回河湖详情,{id:xxx,name:xxx,length:xxx,regions:[{id:xxx,name:xxx, hzUsers:[{id:xxx,name:xxx}, {...}]},{...}],coors:[{...},{...}]}
       * DATA_NOEXIST，河湖不存在
       * FAIL,文件格式错误
13. 下载河湖坐标,/riverSegment/systemAdmin/download/coors,GET
   * **web端使用**
   * **角色：系統管理員**
   * 参数：
       * id,河湖id
   * 返回：
       * OK，下载成功，返回excel流，浏览器保存为excel文件，包含河湖下所有线坐标，每条线一个sheet
       * DATA_NOEXIST，河段不存在
       * FAIL,文件格式错误     
...
7. 编辑扩展项属性,/extra/systemAdmin/attribute/edit
   ...
   * 返回：
	 * OK，保存成功
     * DATA_NOEXIST,扩展项不存在
8. 根据类型取得扩展项列表,/extra/lists/byType,GET
   * **角色：登录有session**
   * 参数： 
	 * type：扩展项类型
   * 返回：
	 * OK，返回扩展项json数组，[{所有基本属性}],不需要attribute
...
1. 取得河段zTree结构json，/segment/systemAdmin/riverResource/tree,GET
    * **角色：所有登录系统的人员，只要有session**
...
17. 移动端原生取得河段詳情,/segment/phone/coors,GET
    * **app端使用**
    * 参数：
        * id:河段id
        * token:原生访问传递的token
...
1. 成员单位用户登录,/unitUser/login,POST
    ...
    * 参数: 
        ...
        * cookie：cookie
    * 返回：
        * OK:登录成功,返回用户属性+成员单位属性，{name:xxx,...}, qqslBucket:xxx, qqslimageBucket:xxx, weChatMenus:[{enum:0, title:新闻动态},{}], articleMenus:[{enum:0, title:新闻动态},{...}, unit:{id:xxx,name:xxx,phone:xxx}]}
...
2. 登录：/hzUser/login,POST
    ...
    * 参数：
        * code:验证码 
    * 返回:
        * OK:登录成功,返回河长详情，管辖的河段 {name:xx,regionId,regionName,phone:xx,job:xx,type:xx,riverSegments:[{id,name:xx,length:xx,beginStation:xx,endStation:xx,riverName:xx,regionId,regionName:xx},{...}], qqslBucket:xxx, qqslimageBucket:xxx, unit:{id:xxx, name:xxx, phone:xxx, type:xxx, typeC:xxx}, weChatMenus:[{enum:0, title:新闻动态},{}], articleMenus:[{enum:0, title:新闻动态},{...}]}
        * 4041: CODE_INVALID 验证码过期
        * 4042: CODE_ERROR 验证码输入错误
...
13. 取得河长通讯录，/hzUser/contact/book,GET
   * **角色：所有登录系统有session**
   * 参数：无
   * 返回:
       * OK,格式：{parentHzUsers:[{id:xxx,name:xxx,phone:xxx,job:xxx},{...}], childHzUsers:[...], siblingHzUsers:[...], unit:{id:xxx, name:xxx, phone:xxx, type:xxx, typeC:xxx}} 
...
11. **投诉统计查看投诉列表,/complaint/statistical/lists,GET`**
   ...
   * 取得投诉列表
   * 参数：
      * @ regionCode: 平台行政区编码, 如果不传从session取
      * period: 今日(today),近7一周(week),近一月(month)
   * 返回:
       * OK，列表，投诉对象所有属性        
12. 根据河湖取得诉列表，/complaint/waterResource/lists,GET
    * **app**
    * **角色：成员单位** 
    * 参数：
        * waterResourceId：河湖id
...
4. 查看巡河详情,/cruise/cruise/{instanceId},GET
    ...
    * 返回：
        * OK，{beginTime,content,path,duration,length, riverSegmentId, riverSegmentName, hzUserId, hzUserName, hzUserPhone, hzbId, hzbName, hzbPhone, cruiseRecords:[{type,description,content,coor,address,instanceId},{..}]}
...
5. 统计月巡河报表，/cruise/report, GET
    * **app端, web端使用**
    * **角色：河长，河长办**
	* 列出平台下月巡河统计情况，按县区行政区统计
    * 参数：
		* date，long类型，包含正确的年份，月份，日期指定为1号
	* 返回：
        * OK，对应河段月巡河统计,json示例
...
7. **巡河统计查看列表,/cruise/statistical/lists,GET`**
   ...
   * 返回:
       * OK，列表，巡河、对象所有属性       
8. 取得自主巡河列表，/cruise/lists/special,GET
    * **app端, web端使用**
    * **角色：各级河长, 巡河员，河长办**
    * 参数：无
    * 返回：
...
## 十八 SectionController, 监测断面
1. 取得监测断面数据列表，/section/record/lists,GET
    * **角色：所有登录用户**
    * 数据每月一条，会将所有断面数据发过来 
    * 参数：
       * id, 监测断面id
    * 返回：
        * OK，{id:xxx, name:xxx, type:xxx, typeC:xxx, riverName(所属河流):xxx, regionName:xxx, targetQuality:xxx, targetQualityC:xxx, datas:[{id:xxx, unitName:xxx, date:xxx, qualityOrdinal:xxx, quality(水质):xxx, qualityC:xxx, }, source(数据来源):xxx, sourceC:xxx, unitUserName:xxx]}
        * DATA_REFUSE, 不是本平台的断面
2. 取得平台下监测断面列表,/section/lists,GET
    * **角色：所有登录用户**
    * 参数：无
    * 返回：
        * OK，[{id:xxx, name:xxx, type:xxx, typeC:xxx, riverName(所属河流):xxx, regionName:xxx, targetQuality:xxx, targetQualityC:xxx}, {...}]
...
河长枚举:
    ...
    // 成员单位角色
    unitUser:simple,unitUser:conservancy,unitUser:hzb,unitUser:hzbAdmin
	
  - 事件编码：M[行政区0-B]190327xxx
    - 行政区：0-西宁，1-大通，2-湟中，3-湟源，4-城东，5-城中，6-城西，7-城北，8-东川工业园，9-甘河工业园，A-南川工业园,B-西宁湟投公司
  - 投诉编码：C[行政区0-B]190327xxx
    - 行政区：0-西宁，1-大通，2-湟中，3-湟源，4-城东，5-城中，6-城西，7-城北，8-东川工业园，9-甘河工业园，A-南川工业园,B-西宁湟投公司	
~~~
