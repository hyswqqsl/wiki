...
6. 取得河湖档案，/river/archive, GET
   ...
   * 返回：
     ...
     * DATA_NOEXIST,河流不存在
	 * DATA_REFUSE, 不是本平台的河流，不能查看	 
7. 新建河湖档案，/river/systemAdmin/archive/create, POST
   * **角色：系统管理员**
   * 参数： id：河流id
   * 返回：
	 * OK，返回河湖档案的所有属性
     * DATA_NOEXIST,河流不存在
	 * DATA_REFUSE, 不是本平台的河流，不能新建档案	 	 
...
6. 取得河湖档案，/waterResource/archive, GET
   ...
   * 返回：
     ...
     * DATA_NOEXIST,水资源不存在
	 * DATA_REFUSE, 不是本平台的水资源，不能查看	 
7. 新建河湖档案，/waterResource/systemAdmin/archive/create, POST
   * **角色：系统管理员**
   * 参数： id：水资源id      
   * 返回：
	 * OK，返回水资源档案的所有属性
     * DATA_NOEXIST,水资源不存在
	 * DATA_REFUSE, 不是本平台的水资源，不能新建档案	 	 
...
3. 系统管理员修改行政区节点，/region/systemAdmin/node/update,POST
   ...
   * 参数：
	 ...
	 * name,节点名
	 * code,编码
	 * level,级别 参考：PROVINCE("省级"), CITY("市州级"), COUNTY("县级"), TOWN("乡镇级"), VILLAGE("村级")
	 * remark,备注
   * 返回：
	 * OK,修改成功
	 * DATA_NOEXIST,id不存在
