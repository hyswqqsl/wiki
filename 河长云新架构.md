...
7. 门户地图取得平台下所有河湖坐标,/waterResource/portal/map/lists,GET
   ...
   * 参数：regionCode: 行政区编码
   * 返回：
      * OK，返回json列表，{rivers:[{id:xxx,name:xxx, remark:xxx, code:xxx, regionLevel:xxx,coors:[]},{...}], lakes:[{id:xxx,name:xxx, remark:xxx, type:xxx,typeC:xxx, code:xxx, regionLevel:xxx, regionLevelC:xxx,coor:{lon:xxx,lat:xxx}}, {...}]}
      * DATA_NOEXIST，平台不存在
      * FAIL,文件格式错误
