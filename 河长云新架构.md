...
10. 取得平台下所有河流线坐标,/river/lineCoors/lists,GET
   ...
   * 参数：
       * platformId,平台id
   * 返回：
       * OK，返回json列表，[{id:xxx,name:xxx,lineCoors:[]},{...}]
       * DATA_NOEXIST，平台不存在
       * FAIL,文件格式错误    
...
6. 取得市级河长名录,/hzUser/hzUserInfo/lists,GET
   * **web端使用**
   * **角色：无**
   * 门户使用
...
4. 门户取得投诉列表,/complaint/portal/lists,GET
    * **角色：无
    * 增加内容缩略contentAbbr，内容前100个字
    * 参数：
        * platformId，平台id
    * 返回：
        * OK，返回投诉列表，只需返回id，createDate, title, contentAbbr, coor, imageUrl，riverSegmentName, status,handleName, handleDate
...
1. 微信取得一河一档列表,/archive/weChat/archive/lists,GET
    * **weChat端使用**
    * **角色：无**   
    * 参数：
       * regionCode:行政区编码     
...
2. 河长取得一河一档列表,/archive/archive/lists,GET
    * **app端使用**
    * **角色：各级河长，巡河员**
...
## 十三  ArticleController 新闻动态和政策方案 
 1. 取得新闻动态列表,/article/newses,GET
     ...
     * 参数：
         * regionCode：行政区编码
     * 返回：
         * OK，所属行政区的新闻动态列表，增加contentAbbr内容缩略100个字
         * FAIL，行政区不存在
