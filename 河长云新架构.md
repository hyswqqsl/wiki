...
1. 取得监测断面数据列表，/section/record/lists,GET
    ...
    * 参数：
       * id, 监测断面id
    * 返回：
        * OK，{id:xxx, name:xxx, type:xxx, typeC:xxx, riverName(所属河流):xxx, regionName:xxx, targetQuality:xxx, targetQualityC:xxx, datas:[{id:xxx, unitName:xxx, date:xxx, quality(水质):xxx, qualityC:xxx, }, source(数据来源):xxx, sourceC:xxx, unitUserName:xxx]}
