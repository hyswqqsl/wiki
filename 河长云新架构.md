...
1. 新建河湖, /waterResource/systemAdmin/create, POST
   * **角色：系统管理员**
   * 如果是顶级节点，如西宁平台的湟水河，或湖泊水库，pId为0
   * 如果是类型是河流，不需要coor点坐标
   * 参数：
     * regionCode:行政区编码
     * name,节点名
     * remark,备注     
     * pId: 上级pId
     * type: 河流，RIVER，湖泊，LAKE，水库，RESERVOIR，水源地，WATER_PLACE，灌区，IRRIGATION_AREA
     * regionLevel: 行政区级别
     * @ coor：非河流，如湖泊，水库有点坐标 
   * 返回：
     * OK: 新建成功
...
10. 取得河湖档案全部属性,/waterResource/details/{id},GET
   ...
   * 参数： id：河湖id
   * 返回：
	 * OK，返回河湖所有属性，河湖档案的所有属性，河湖档案下所有属性(基础+动态)，根据xml配置的结构返回json
     * DATA_NOEXIST,河流不存在	
11. 地图取得河流二级图层数据，/waterResource/river/second/details/{id},GET
   * **角色：所有登录系统的人员，只要有session**
   * 参数： id：河湖id
   * 返回：
	 * OK，返回河湖所有坐标属性，名称，备注，类型，code等
     * DATA_NOEXIST,河流不存在
12. 地图取得湖泊水库二级图层数据，/waterResource/lake/second/details/{id},GET
   * **角色：所有登录系统的人员，只要有session**
   * 参数： id：河湖id
   * 返回：
	 * OK，返回河湖所有坐标属性，名称，备注，类型，code等
...
12. 删除河段，/segment/systemAdmin/delete/{id},DELETE
   ...
   * 返回：
       ...
       * DATA_NOEXIST，水资源不存在
       * FAIL，删除失败，水资源下有数据
1. 取得河段详情, /segment/segment/{id},GET
    * **app端使用**
    * **角色：各级河长**
    * 增加河段对应的一河一档兴趣点
    * 参数：id:河段
...
4. 取得河段列表，/segment/region/lists, GET
    * **weChat使用**
    * **角色：weChat**
    * 参数：
        * regionCode：行政区编码, 这个regionCode是当先位置的县区级行政编码
    * 返回：
        * OK，行政区下的所有河段，[{id,name},{..}]，如果行政区下没有河道，返回流过市州的所有河流id和name      2. 取得河长管辖的河段,/segment/lists,GET
    * **app端使用**
    * **角色：各级河长**
    * 参数：无
    * 返回：
...
3. 移动端原生取得河段的面坐标,/segment/phone/coors,GET
    * **app端使用**
    * 参数：
        * id:河段id
        * token:原生访问传递的token
...
5. 河长办通过水利一张图查看河道列表，/segment/hzb/map/lists
   * **web端使用**
   * 角色：各级河长办人员
...
6. 取得水利一张图河段详情, /segment/hzb/map/details/{id},GET
   * **web端使用**
   * 角色：各级河长办人员
   * 参数：id:河段id
   * 返回：
...
11. 门户取得平台下湖长列表,/hzUser/portal/lake/lists,GET
   ...
   * 返回:
       * OK,格式：[{name:xxx,job:xxx,level:xxx,levelC:xxx,riverSegments:[{id:xxx,name:xxx}]}, {...}]  
12. 工作台取得河长，河流数量，/hzUser/workspace/hz/river/num,GET
   * **角色：所有登录系统有session**
...
## 水利地图使用的接口
7. 门户地图取得平台下所有河湖坐标,/waterResource/portal/map/lists,GET
11. 地图取得河流二级图层数据，/waterResource/lake/second/details/{id},GET
12. 地图取得湖泊水库二级图层数据，/waterResource/lake/second/details/{id},GET