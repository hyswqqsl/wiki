...
4. 编辑河湖档案属性,/waterResource/systemAdmin/attribute/edit
   ...
   * 返回：
	 * OK，保存成功
     * DATA_NOEXIST,河湖不存在	
4. 系统管理员取得河湖详情: /waterResource/systemAdmin/details/{id}, GET
   * **角色：系统管理员**
   * 参数： id：河流id
   * 返回：
     * OK，返回河湖所有属性，河湖档案的所有属性，河湖档案下所有属性(基础+动态)，根据xml配置的结构返回json及，所有坐标
     * DATA_NOEXIST,河流不存在        
5. 删除河湖节点，/waterResource/systemAdmin/node/delete/{id},DELETE
   * 角色：系统管理员
   * 如果节点不是叶子节点，不能删除
   ...
   * 返回:
      ...
      * DATA_NOEXIST,id不存在
      * FAIL，删除失败，河湖下有数据	 
6. 门户地图取得平台下所有河湖坐标,/waterResource/portal/lists,GET
   * 角色：无
   * 河流是中心线坐标，非河流是点坐标
   * 参数：
   * 返回：
      * OK，返回json列表，{rivers:[{id:xxx,name:xxx,coors:[]},{...}], lakes:[{id:xxx,name:xxx,type:xxx,typeC:xxx, coor:{lon:xxx,lat:xxx}}, {...}]
      * DATA_NOEXIST，平台不存在
      * FAIL,文件格式错误
7. 门户取得河湖列表,/waterResource/portal/lists,GET
    * **角色：无**   
    ...
    * 参数：
       * platformId:平台id
    * 返回：
        * OK,返回一河一档列表,每个对象包含,id, name, introduce, type,typeC, createTime
8. 门户取得河湖档案，/waterResource/portal/details/{id}, GET
   * **角色：无**
   * 返回：
	 * OK，返回河流(水资源)所有属性，河湖档案的所有属性，基础属性
     * DATA_NOEXIST,河流不存在
	 * DATA_REFUSE, 不是本平台的河流，不能查看    
9. 取得河湖档案全部属性,/waterResource/details/{id},GET
