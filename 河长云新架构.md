...
- [河长云新架构接口](#%E6%B2%B3%E9%95%BF%E4%BA%91%E6%96%B0%E6%9E%B6%E6%9E%84%E6%8E%A5%E5%8F%A3)
  ...
  - [十一 河长 HzUserController](#%E5%8D%81%E4%B8%80-%E6%B2%B3%E9%95%BF-hzusercontroller)
  - [十二 投诉，ComplaintControler](#%E5%8D%81%E4%BA%8C-%E6%8A%95%E8%AF%89complaintcontroler)
  - [十三 ArchiveController,一河一档控制层](#%E5%8D%81%E4%B8%89-archivecontroller%E4%B8%80%E6%B2%B3%E4%B8%80%E6%A1%A3%E6%8E%A7%E5%88%B6%E5%B1%82)
  - [十四  ArticleController 新闻动态和政策方案](#%E5%8D%81%E5%9B%9B--articlecontroller-%E6%96%B0%E9%97%BB%E5%8A%A8%E6%80%81%E5%92%8C%E6%94%BF%E7%AD%96%E6%96%B9%E6%A1%88)
  - [十五 MatterController，事件管理](#%E5%8D%81%E4%BA%94-mattercontroller%E4%BA%8B%E4%BB%B6%E7%AE%A1%E7%90%86)
  - [十六 CruiseController 巡河控制层](#%E5%8D%81%E5%85%AD-cruisecontroller-%E5%B7%A1%E6%B2%B3%E6%8E%A7%E5%88%B6%E5%B1%82)
...
## 河长云类结构
![新架构14](/uploads/3be22d335d54b3a446fee11fadba3d13/新架构14.jpg)
...
4. 成员单位用户修改密码:在基本资料的修改密码处点击保存时调用， /unitUser/updatePassword POST
   ...
   * 返回：
       * OK:修改成功
       * FAIL:当前密码错误    
7. 成员单位用户注销,/unitUser/logout POST
   * **web端使用**
   * **角色：成员单位用户**
   * 参数：无
   * 返回：OK,注销成功       
...
7. 门户取得行政区下河长列表，/hzUser/portal/lists,GET
   * **角色：无**
   * 只返回河长，不返回巡河员      
...
8. 取得行政区下河长列表，/hzUser/lists,GET
   ...
   * 参数：
       ...
       * level: 级别，所有，市级，县级，乡级，村级       
   * 返回:
       * OK,格式：[{name:xxx,job:xxx,level:xxx,levelC:xxx, role:xxx,roleC:xxx}, {...}] 
...
10. 投诉统计，/complaint/statistical,GET
   ...
   * 返回:
       * OK，json:{regions:[{name:"城东区",id:xxx,num(总数量):xxx，reviewed(已处理数量):xxx,pending(待处理数量):xxx}, {...}],    riverBody("水体问题数量"):xxx, riverSurface("河面问题数量"):xxx,riverMiddle("河中问题数量"):xxx,riverSide("河岸问题数量"):xxx,activity("涉水活动问题数量"):xxx,outlet"排污口问题数量"):xxx,billboard("河长牌问题数量"):xxx,build("水工建筑物问题数量")}        
        
## 十三 ArchiveController,一河一档控制层
...
## 十四  ArticleController 新闻动态和政策方案 
...
16. 事件统计，/matter/statistical,GET
   ...
   * 参数：period: 今日(today),近7一周(week),近一月(month)
   * 返回:
       * OK，json:{regions:[{name:"城东区",id:xxx,num(总数量):xxx,complete(已办结数量):xxx,handle(承办中数量):xxx,deliver(代办数量):xxx}, {...}],            superAssign("上级交办数量"):xxx,systemPush("系统推送数量"),supervise("社会媒体监督数量"),complaint("群众投诉数量"),cruise("河长巡河发现数量"),hzTast("河长下达事件数量")}
         
## 十六 CruiseController 巡河控制层
...
6. 巡河统计，/curise/statistical,GET
   ...
   * 参数：period: 今日(today),近7一周(week),近一月(month)
   * 返回:
       * OK，json:{regions:[{name:"城东区",id:xxx,num(巡河次数):xxx，hzUser(巡河人数):xxx，duration("巡河时长"):xxx,length("巡河公里数"):xxx, record("发现问题数"):xxx}, {...}],    riverBody("水体问题数量"):xxx, riverSurface("河面问题数量"):xxx,riverMiddle("河中问题数量"):xxx,riverSide("河岸问题数量"):xxx,activity("涉水活动问题数量"):xxx,outlet"排污口问题数量"):xxx,billboard("河长牌问题数量"):xxx,build("水工建筑物问题数量")}   
