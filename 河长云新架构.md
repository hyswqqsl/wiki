...
- [河长云新架构接口](#%E6%B2%B3%E9%95%BF%E4%BA%91%E6%96%B0%E6%9E%B6%E6%9E%84%E6%8E%A5%E5%8F%A3)
  ...
  - [十七 信息收发，UnitContactController](#%E5%8D%81%E4%B8%83-%E4%BF%A1%E6%81%AF%E6%94%B6%E5%8F%91unitcontactcontroller)
  - [十八 PanoramaController，全景](#%E5%8D%81%E5%85%AB-panoramacontroller%E5%85%A8%E6%99%AF)
...
7. 门户取得行政区下河长列表，/hzUser/portal/he/lists,GET
   * **角色：无**
   * 只返回河长，不返回巡河员      
...
8. 门户取得平台下湖长列表,/hzUser/portal/hu/lists,GET
   * **角色：无**
   * 取得平台下级河长列表，不包含巡河员
   ...
   * 返回:
       * OK,格式：[{name:xxx,job:xxx,level:xxx,levelC:xxx,riverSegments:[{id:xxx,name:xxx}]}, {...}]  
9. 工作台取得河长，河流数量，/hzUser/workspace/hz/river/num,GET
   * **角色：所有登录系统有session**
   ...
   * 参数：
       * platformId:平台id 
   * 返回:
       * OK,格式：{hzUsers:[{type:"river", city(市级数量):xxx，county(县级数量):xxx, town(乡级数量):xxx, village(村级数量):xxx}, {...}], rivers:[{type:"river",city(市级数量):xxx，county(县级数量):xxx}, {...}]}
...
16. 事件统计，/matter/statistical,GET
   ...
   * 参数：period: 今日(today),近7一周(week),近一月(month)
   * 返回:
       * OK，json:{regions:[{name:"城东区",id:xxx,num(总数量):xxx,complete(已办结数量):xxx,handle(承办中数量):xxx,deliver(待代办数量):xxx}, {...}], waitNum(待办数量):xxx}
