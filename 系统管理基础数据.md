...
3. 删除一河一档，/riverArchive/admin/delete/{id},DELETE
   ...
   * 返回：
     * OK，删除成功
     * DATA_NOEXIST,一河一档不存在
4. 取得一河一档,/riverArchive/admin/details,GET
   * **web端使用**
   * **角色：市河长办管理员**
   * 参数：
     * riverId, 河流id
   * 返回：
     * OK，一河一档对象详情，如果一河一档不存在，返回空json
     * DATA_NOEXIST,河流不存在     
...
2. 取得河段详情，/riverSegment/admin/details/{id},GET
      * id: 河段id
      * OK，河段属性+所属行政区列表+每个行政区下河长列表,{id:xxx,name:xxx,length:xxx,regions:[{id:xxx,name:xxx, hzUsers:[{id:xxx,name:xxx}, {...}]},{...}]}        
3. 新建河段，/riverSegment/admin/create,POST
   * **web端使用**
   * **角色：市河长办管理员**
   * 如果是顶级节点，如西宁市河段，pId为0
   * 如果不是顶级节点，断上级节点是否有行政区，如果没有，不能保存，返回DATA_LOCK
   * 参数：
       * name, 河段名称
       * pId，父级节点id
       * riverId，河流id
   * 返回：
       * OK，新建成功
       * DATA_LOCK 上级节点没有行政区
4. 编辑河段，/riverSegment/admin/update,POST
   * **web端使用**
   * **角色：市河长办管理员**
   * 如果是顶级节点，如西宁市河段，pId为0
...
5. 取得所属行政区，/riverSegment/admin/region/lists,GET
   * **web端使用**
   * **角色：市河长办管理员**
   ...
   * 返回：
       * OK，行政区list
6. 取得行政区下的河长列表，/riverSegment/admin/hzUser/lists,GET
   * **web端使用**
   * **角色：市河长办管理员**
   * 参数：
       * regionId,行政区id
   * 返回：
       * OK，行政区下的河长list
       * DATA_NOEXIST，行政区不存在
7. 删除河段，/riverSegment/admin/delete/{id},DELETE
   * **web端使用**
   * **角色：市河长办管理员**
   * 如果不是叶子节点，不能删除
   * 如果河段下有巡河，不能删除
   ...
   * 返回：
       ...
       * DATA_NOEXIST，河段不存在
       * FAIL，删除失败，河段下有数据
8. 上传河段坐标,/riverSegment/admin/uploadCoor,POST
   * **web端使用**
   * **角色：市河长办管理员**
    * 参数：
       * id,河段id
       ...
       * type，0-线坐标，1-面坐标
   * 返回：
       * OK，上传成功，返回河段coors,{coors:[{lon:xx,lat:xx,ele:xx},{...}],type:"线坐标"}
       * DATA_NOEXIST，河段不存在
       * FAIL,文件格式错误       
