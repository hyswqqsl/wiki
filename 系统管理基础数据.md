...
8. 删除河段，/riverSegment/admin/delete/{id},DELETE
   ...
   * 返回：
       ...
       * DATA_NOEXIST，河段不存在
       * FAIL，删除失败，河段下有数据
9. 上传河段左岸坐标,/riverSegment/admin/upload/leftCoor,POST
   * **web端使用**
   * **角色：市河长办管理员**
    * 参数：
       * id,河段id
       * file,上传的excel文件
   * 返回：
       * OK，上传成功，返回河段coors,{leftCoors:[{lon:xx,lat:xx,ele:xx},{...}], rightCoors:[{lon:xx,lat:xx,ele:xx}, {...}]}
       * DATA_NOEXIST，河段不存在
       * FAIL,文件格式错误
10. 上传河段右岸坐标,/riverSegment/admin/upload/rightCoor,POST
   * **web端使用**
   * **角色：市河长办管理员**
    * 参数：
       * id,河段id
