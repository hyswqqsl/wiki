...
2. 取得河段详情，/riverSegment/admin/details/{id},GET
      * id: 河段id
      * OK，河段属性+所属行政区列表+每个行政区下河长列表+巡河员列表,{id:xxx,name:xxx,length:xxx,regions:[{id:xxx,name:xxx, hzUsers:[{id:xxx,name:xxx}, {...}]},{...}],patrolUsers:[{id:xx,name:xx,phone:xx},{...}]areaCoors,leftCoors,rightCoors}
...
9. 上传河段坐标,/riverSegment/admin/upload/coors,POST
   * **web端使用**
   * **角色：市河长办管理员**
   * excel包含3个sheet，面，左岸，右岸
   * sheet有可能是1个(面)，或是3个(面，左岸，右岸)，后台根据sheet名导入数据
   * 参数：
       * id,河段id
       * file,上传的excel文件
   * 返回：
