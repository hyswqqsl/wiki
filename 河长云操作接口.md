...
![class](/uploads/0b40f2f7fc1c1b27c8bac7d554236e33/class.png)
...
2. 移动端微信用户登录,/weChat/login,POST
   ...
   * 参数：
       ...
       * regionCode:行政区编码       
   * 返回
       * OK，登录成功  {qqslBucket:xxx, qqslimageBucket:xxx}
3. 移动端取得公众号定制信息,/weChat/phone/menu,GET
    * **app端使用**
    * **角色：无**
    * 移动端未登录时，需要动态显示新闻菜单，所以要去的菜单配置信息 
   * 参数：
       * regionCode:行政区编码       
   * 返回
       * OK，{level:xxx, weChatMenus:[{enum:0, title:新闻动态},{}], articleMenus:[{enum:0, title:新闻动态},{...}}
