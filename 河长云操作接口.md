...
4. 取得河长数量，/hzUser/num,GET
   * **weChat,web端使用**
   * **角色：无**
   * 参数：
       * regionCode：行政区编码    
   * 返回： 
       * OK，格式:{master(总河长):xxx,slave(副总河长):xxx,city:34,county:123,town:xxx,villlage:xxx}
5. 取得行政区下河长及管理河道列表，/hzUser/hzUserInfo/lists,GET
   * **weChat,web端使用**
   * **角色：无**
   ...
   * 参数：
       * regionCode：行政区编码    
   * 返回:
       * OK,格式：[{name:xxx,job:xxx,riverSegments:xxx}, {...}]
6. 取得下级行政区列表及河长数量，/hzUser/region/down/lists,GET
   * **weChat,web端使用**
   * **角色：无**
   ...
   * 参数：
       * regionCode：行政区编码    
   * 返回：
       * OK,格式：[{name:xxx,num:xxx},{...}]                        
7. 村级河长直接发报告给乡级河长办,/hzUser/village/report,POST
      * 图片单独上传
      * 村级报告type=0
      * **app端使用**
      * **角色：村级河长**
...
9. 取得河长办报告详情,/hzUser/report/{instanceId}，GET
   * **app,web端使用**
   * **角色：村级，乡级河长，县河长办**
   * 如果是村级河长，必须是自己的河长报告
...
10. 村河长删除自己的报告，/hzUser/report/delete/{instanceId},DELETE
   * **app端使用**
   * **角色：村级河长**
   ...
   * 返回：
       * OK，删除成功
       * 4010: UNAUTHORIZED,不是属于自己的报告
11. 村河长编辑自己的报告，/hzUser/report/update,POST
   * **app端使用**
   * **角色：村级河长**
...
14. 删除河长办任务，/hzUser/task/delete/{instanceId},DELETE
   ...
   * 返回：
       * OK，删除成功
       * 4010: UNAUTHORIZED,不是属于自己的任务
15. 取得河长发的任务列表,/hzUser/task/lists,GET
   * **app端使用**
   * **角色：市，县，乡级河长**
   * 参数：无
   * 返回：
       * OK，任务列表
16. 取得任务详情,/hzUser/task/details/{instanceId},GET
   * **web, app端使用**
   * **角色：市，县，乡级河长**
   * 河长自己的任务，能取得
   * 河长办上级河长发的任务，能取得
