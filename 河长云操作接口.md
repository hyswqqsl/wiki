...
5. 取得下级行政区列表及河长数量，/hzUser/region/down/lists,GET
   ...
   * depth的含义：0-只取本级河长数量，1-本级加下级，2-本级加下级再加孙子级
   * 参数：
       * regionId：行政区Id
       * depth: 0-3    
   * 返回：
...
1. 河长办用户登录,/hzbUser/login,POST
    ...
    * 参数: 
        ...
        * cookie：cookie
    * 返回：
        * OK:登录成功,返回河长办用户属性+河长办属性，{name:xxx,...,hzb:{name：xxx,regionCode:xxx,...}, qqslBucket:xxx, qqslimageBucket:xxx}
...
2. 移动端微信用户登录,/weChat/login,POST
   ...
   * 参数：
       ...
       * regionCode:行政区编码       
   * 返回
       * OK，登录成功  {unionId:xxx, name:xxx, qqslBucket:xxx, qqslimageBucket:xxx, level:xxx, weChatMenus:[{enum:0, title:新闻动态},{}], articleMenus:[{enum:0, title:新闻动态},{...}]}
