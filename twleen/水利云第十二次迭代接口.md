...
1. 取得工单列表:/cruise/app/ticket/lists,GET
  ...
  * 参数：   
	* projectId: 项目id
  * 返回:	
	* OK，返回列表数组json,每项包含：id，type(巡查类型),typeC,status(未完成，已完成),statusC,title,content，accountId(子账号id)，accountName(子账号名),beginDate,endDate,coor(坐标json),address
2. 取得巡查列表，/cruise/app/cruise/lists,GET
  ...
  * 参数：
    * projectId：项目id
  * 返回：
* OK，返回列表数组json,每项包含：id，type(日常，临时),typeC,resulet(巡查结果),duration(时长)，length(里长),accountId, accountName,beginTime,endTime
